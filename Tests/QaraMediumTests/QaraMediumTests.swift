...
final class QaraMediumTests: XCTestCase {
    ...
    func testCreatingPost() async {
        ...
        if let me {
            let article = QaraMediumArticle(
                title: "Hello World from Xcode",
                content: "This is a draf article. No content. Only testing purpose. To be deleted!"
            )
    ...
    func testCreatingAIPost() async {
        let article = QaraMediumArticle(
            title: "Bind: Learn a Language with AI",
    ...
    func testGetPublications() async {
        let expectationGettingPosts = expectation(description: "Request - get publications")

        var posts: [QaraMediumItem]?
        switch await sut.getPosts(username: "salgarayes") {
        case .success(let data):
            print(data)
            posts = data.items
        case .failure(let failure):
        ...
        expectationGettingPosts.fulfill()

        await fulfillment(of: [expectationGettingPosts])
